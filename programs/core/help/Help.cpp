...
HelpCommand::HelpCommand() : Command{"help", -1, "A program that fetches information about specified commands"}
{
...
int HelpProgram::run(std::vector<std::string> cmd_args)
{
    ...
    for (int arg_index = 0; arg_index < cmd_args.size(); arg_index++)
    {
        ...
        {
            if (arg_index != 0 &&
                CommandRegistry::hasCommand(cmd_args.at(arg_index - 1))) // for printing \n in a smart way
            {
                std::cout << std::endl;
            }
