...
FileSystemObject *objectLocator(DirectoryObject *root_dir, DirectoryObject *current_dir, std::string path)
{
    ...
    {
        mover = current_dir;
    } // In any else case, traversal starts from current directory
    ...
    while (index < length) // If the path is correct,
                           // index is exactly equal to length
    {
        ...
        if (path.at(index) == '/')
        {
            ...
            /*
                If no cases above are handled, it is time to check the current
                subpath whether it is in the mover drectory. If a path is correct, then
                there is a solution.
            */
    ...
    /*
        ...
        That means that even at the last subpath, index is incremented and
        then loop is exited.
    */
    if (index != length)
    {
...
FileSystemObject::FileSystemObject(std::string inp_name, DirectoryObject *inp_parent, bool inp_is_dir)
...
std::string FileSystemObject::getCreatedTime() const
{
    return created_at;
}
...
int FileSystemObject::displayInfo()
{
    ...
    std::cout << "Parent Directory: " << ((getParent()) ? getParent()->getName() : "null") << "/" << std::endl;
...
DirectoryObject::DirectoryObject(std::string inp_name, DirectoryObject *inp_parent)
    : FileSystemObject(inp_name, inp_parent, true)
{
}
...
DirectoryObject *DirectoryObject::create(std::string name, DirectoryObject *parent)
{
...
FileObject::FileObject(std::string inp_name, DirectoryObject *inp_parent)
    : FileSystemObject(inp_name, inp_parent, false)
{
}

FileObject::~FileObject() = default;

FileObject *FileObject::create(std::string name, DirectoryObject *parent)
{
    if (nameValidator(name) && parent)
    {
